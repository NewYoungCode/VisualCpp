...
namespace Time {
	namespace Now {
		inline  std::string ToString(const std::string &format="yyyy-MM-dd hh:mm:ss") {
			...
			std::string  Month = std::to_string((WORD)time.wMonth);//月
			std::string  Day = std::to_string((WORD)time.wDay);//日
			std::string Hour = std::to_string((WORD)time.wHour);//时
			std::string Minute = std::to_string((WORD)time.wMinute);//分
			std::string Second = std::to_string((WORD)time.wSecond);//秒
			std::string wMilliseconds = std::to_string((WORD)time.wMilliseconds);//毫秒
